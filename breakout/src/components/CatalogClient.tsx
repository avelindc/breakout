    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-blue-900/40 to-blue-800/20 backdrop-blur-xl rounded-3xl p-6 border border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <p className="text-blue-200/70 font-medium text-sm tracking-wide">
            {total.toLocaleString("id-ID")} Lagu Ditemukan
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {songs.map((song, i) => (
            <div 
              ...
              className="group relative bg-white/[0.03] backdrop-blur-sm border border-white/5 rounded-3xl p-5 cursor-pointer hover:bg-white/[0.08] hover:border-white/20 hover:-translate-y-1 transition-all duration-300 overflow-hidden shadow-lg hover:shadow-blue-900/20"
            >
              <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-600/0 via-blue-600/0 to-blue-600/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              
              <div className="relative z-10">
                <div className="flex items-start gap-4 mb-4">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center flex-shrink-0 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 border border-white/10 shadow-inner">
      ...
      {!loading && hasMore && songs.length > 0 && (
        <div className="flex justify-center pt-8 pb-12">
          <button 
            onClick={loadMore}
            className="px-8 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-bold hover:bg-white/10 hover:scale-105 active:scale-95 transition-all shadow-lg backdrop-blur-md flex items-center gap-2"
          >
            Muat Lebih Banyak
          </button>
        </div>
          ...
          <div className="relative z-10 bg-gradient-to-b from-[#1c2331] to-[#121620] rounded-[2rem] max-w-lg w-full border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)] transform animate-scale-up flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="relative shrink-0 h-48 bg-gradient-to-br from-blue-600/30 via-purple-600/20 to-black/40 flex items-end p-8 border-b border-white/5">
              ...
              <div className="relative z-10 flex items-center gap-5 w-full">
                <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-xl border border-white/20 transform -rotate-3">
                  <Music className="w-10 h-10 text-white drop-shadow-md" />
                </div>
                <div className="flex-1 min-w-0">
                  <h2 className="text-2xl sm:text-3xl font-black text-white truncate drop-shadow-md">{selectedSong.title}</h2>
                  <p className="text-blue-200 font-medium text-lg truncate drop-shadow-md opacity-90 mt-1">{selectedSong.artist}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8 space-y-6 overflow-y-auto overscroll-contain">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/5 hover:bg-white/10 p-5 rounded-2xl border border-white/5 transition-colors">
                  <p className="text-xs text-blue-300/60 uppercase tracking-wider font-bold mb-1.5 flex items-center gap-1.5">
                    <Mic2 className="w-3.5 h-3.5"/> Vokal
                  </p>
                  <p className="text-base text-white font-semibold truncate">{selectedSong.vokal || "-"}</p>
                </div>
                <div className="bg-white/5 hover:bg-white/10 p-5 rounded-2xl border border-white/5 transition-colors">
                  <p className="text-xs text-purple-300/60 uppercase tracking-wider font-bold mb-1.5 flex items-center gap-1.5">
                    <Building className="w-3.5 h-3.5"/> Publisher
                  </p>
                  <p className="text-base text-white font-semibold truncate">{selectedSong.publisher || "-"}</p>
                </div>
              </div>
              ...
              {selectedSong.driveLink ? (
                <a 
                  ...
                  className="group relative w-full py-4 rounded-2xl bg-white text-blue-900 font-bold hover:scale-[1.02] active:scale-[0.98] transition-all flex justify-center items-center gap-2 overflow-hidden shadow-[0_0_30px_rgba(255,255,255,0.15)]"
                >
